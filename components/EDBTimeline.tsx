...
const getEventStatus = (edb: EDBTimelineProps['edb'], keyEvent: KeyEvent) => {
  ...
  switch (keyEvent) {
    case 'CREATION':
      return latestEvent.eventType === 'SUBMITTED' ? 'COMPLETED' : 'IN_PROGRESS';
    case 'APPROVAL':
      if (['APPROVED_DG', 'APPROVED_DIRECTEUR'].includes(latestEvent.eventType as EDBEventType)) return 'COMPLETED';
      if (latestEvent.eventType === 'REJECTED') return 'REJECTED';
      if (['APPROVED_RESPONSABLE', 'ESCALATED'].includes(latestEvent.eventType as EDBEventType)) return 'IN_PROGRESS';
      return 'PENDING';
    case 'MAGASINIER':
      return latestEvent.eventType === 'MAGASINIER_ATTACHED' ? 'COMPLETED' : 'PENDING';
    case 'SUPPLIER':
      return latestEvent.eventType === 'SUPPLIER_CHOSEN' ? 'COMPLETED' : 'PENDING';
    case 'IT_APPROVAL':
      return latestEvent.eventType === 'IT_APPROVED' ? 'COMPLETED' : 'PENDING';
    case 'FINAL_APPROVAL':
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {status === 'COMPLETED' ? 'Terminé' : 
                 status === 'IN_PROGRESS' ? (keyEvent === 'CREATION' && edb.currentStatus !== 'DRAFT' ? 'Terminé' : 'En cours') : 
